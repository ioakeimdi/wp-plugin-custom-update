...
class WP_Plugin_Custom_Update {
	...
	public function get_current_version() {
		...
		if ( empty( $plugin_data['Version'] ) ) {
    		return new WP_Error( 'invalid_plugin', 'Could not retrieve plugin version.' );
		}
	...
	public function get_remote() {
		...
		if ( is_wp_error( $response ) ) {
			return new WP_Error( 'http_request_failed', 'Request failed: ' . $response->get_error_message() );
		}
		...
		if ( json_last_error() !== JSON_ERROR_NONE ) {
			return new WP_Error( 'json_parse_error', 'Invalid JSON: ' . json_last_error_msg() );
		}
		...
		switch ( $action ) {
			case 'version':
				if ( isset( $data['version'] ) ) {
					...
					return new WP_Error( 'version_missing', 'The "version" field is missing in the JSON file.' );
				}
				break;
			case 'download_url':
				if ( isset( $data['download_url'] ) ) {
					...
					return new WP_Error( 'download_url_missing', 'The "download_url" field is missing in the JSON file.' );
				}
				break;
		}
	}
